...
Run35073227338 /31ee0a8 completed the first full Java/Go graph scan:758 coordinates; the public page exposed10 matched dependency annotations, with multiple advisory aliases. This is a displayed lower bound, NOT a verified full finding count: GitHub limits annotations. Read the complete vulnerability-review.json artifact before counting or triaging all findings. It correctly stopped before signing or actual APK installation. Java-only87-component review had no known matches. A source-module match does not establish that the module is linked into the Android APK.
...
A separately installed official `golang.org/x/vuln/cmd/govulncheck@v1.8.0` binary-mode symbol scan of PRIOR1.28.5 ARM64 sing-box produced14 advisory IDs, including emitted function-labelled findings for chi middleware, x/crypto, x/mod and gRPC. Raw JSON stream, tool identity, summary and advisory records are retained. Inspection of govulncheck v1.8.0 internal/vulncheck/binary.go showed that stripped binaries can trigger allKnownVulnerableSymbols: function fields can be conservative module-level fallback, NOT actual symbol-presence evidence. These outputs cannot be used to assert either exploitation or source-level reachability. The module-version matches still require review. These are PRIOR-binary results, not a completed candidate scan. No runtime finding has been waived.

A direct OSV query independently matched x/crypto@v0.54.0 and grpc@v1.79.1, confirming additional runtime-coordinate matches. They may already be in the complete original report but omitted from GitHub’s annotation display. Do not infer source/binary version divergence or scanner-feed mismatch from truncated annotations.
...
Native build.py currently vendors corresponding dependencies AFTER building the ELF. The complete source graph has not yet been compared with candidate binary tables. Investigate/freeze dependency resolution before compilation and verify whether the vendored source exactly matches actual compiled module versions; do not assert a specific root cause or source reproducibility before testing. A future change should build from a frozen vendor graph and rerun all native gates, not silently refresh modules merely to clear warnings.
