...
public class StartupDiagnosticsTest {
 ...
 @Test public void observedHttpResponseIsDistinctFromSelectedRemoteProof(){AtomicLong now=new AtomicLong();StartupDiagnostics.Attempt a=StartupDiagnostics.begin(0,0,now::get);a.coreStarted();now.set(120_000_000);a.probeFinished(true,100);assertEquals(StartupDiagnostics.Confirmation.RESPONSE_SEEN,a.confirmation());assertTrue(a.report().contains("selected_remote_route=NOT_PROVEN"));}
 @Test public void supersededAttemptCannotChangeCurrentReport(){AtomicLong now=new AtomicLong();StartupDiagnostics.Attempt old=StartupDiagnostics.begin(0,1,now::get);old.coreStarted();StartupDiagnostics.Attempt current=StartupDiagnostics.begin(0,2,now::get);current.coreStarted();old.probeFinished(true,10);old.received(100);assertEquals(StartupDiagnostics.Confirmation.CHECKING,StartupDiagnostics.confirmation());assertFalse(StartupDiagnostics.safeReport().contains("probe_duration_ms=10"));}
 @Test public void stoppedAttemptCannotBecomeConfirmedLater(){AtomicLong now=new AtomicLong();StartupDiagnostics.Attempt a=StartupDiagnostics.begin(0,0,now::get);a.coreStarted();a.stop();a.probeFinished(true,10);assertEquals(StartupDiagnostics.Confirmation.UNCONFIRMED,a.confirmation());assertTrue(a.report().contains("session=STOPPED"));}
