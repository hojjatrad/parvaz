...
public class LatencyResultTest {
 @Test public void unknownAndFailuresAreNotUntested(){assertEquals(LatencyResult.FAILED,LatencyResult.measured(-1));assertEquals(LatencyResult.UNCONFIRMED,LatencyResult.measured(-2));assertEquals(LatencyResult.BUSY,LatencyResult.measured(ProbeAdmission.BUSY));assertEquals(LatencyResult.FAILED,LatencyResult.measured(0));assertEquals(LatencyResult.NETWORK_CHANGED,LatencyResult.scoped(LatencyResult.FAILED,false));assertEquals(LatencyResult.NETWORK_CHANGED,LatencyResult.scoped(42,false));assertEquals(LatencyResult.CANCELLED,LatencyResult.scoped(LatencyResult.CANCELLED,false));}
 @Test public void localSetupAndUnprovableRoutesNeverBecomeServerFailureOrPositive(){assertEquals(LatencyResult.START_ERROR,LatencyResult.measured(-25));assertEquals(LatencyResult.ROUTE_UNVERIFIED,LatencyResult.measured(-26));assertTrue(LatencyResult.START_ERROR<0);assertTrue(LatencyResult.ROUTE_UNVERIFIED<0);}
