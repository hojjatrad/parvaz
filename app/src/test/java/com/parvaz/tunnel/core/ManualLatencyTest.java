...
public class ManualLatencyTest {
 ...
 @Test public void unknownAndExceptionGetVisibleTerminalStates()throws Exception{
  List<Profile> rows=profiles(3);PingManager m=manager(p->{if(p.id.endsWith("0"))return -2;if(p.id.endsWith("1"))throw new IllegalStateException("fixture");return ProbeAdmission.BUSY;});Listener l=new Listener();m.startBatch(rows,l);until(()->l.finished==1);assertEquals(LatencyResult.UNCONFIRMED,rows.get(0).ping);assertEquals(LatencyResult.FAILED,rows.get(1).ping);assertEquals(LatencyResult.BUSY,rows.get(2).ping);
 }
