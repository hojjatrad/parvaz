...
public class ManualLatencyTest {
 ...
 @Test public void unknownAndExceptionGetVisibleTerminalStates()throws Exception{
  List<Profile> rows=profiles(4);PingManager m=manager(p->{if(p.id.endsWith("0"))return -2;if(p.id.endsWith("1"))throw new IllegalStateException("fixture");if(p.id.endsWith("2"))return ProbeAdmission.BUSY;return -1;});Listener l=new Listener();m.startBatch(rows,l);until(()->l.finished==1);assertEquals(LatencyResult.UNCONFIRMED,rows.get(0).ping);assertEquals(LatencyResult.START_ERROR,rows.get(1).ping);assertEquals(LatencyResult.BUSY,rows.get(2).ping);assertEquals(LatencyResult.FAILED,rows.get(3).ping);
 }
 ...
 @Test public void networkChangeRejectsOldMeasurement()throws Exception{
  Profile p=profiles(1).get(0);PingManager m=manager(ignored->{NetworkEpoch.changed();return 100;});Listener l=new Listener();m.testOne(p,l);until(()->l.finished==1);assertEquals(LatencyResult.NETWORK_CHANGED,p.ping);
 }
 ...
 @Test public void liveResultDoesNotReplacePendingManualTest(){Profile p=profiles(1).get(0);ProfileStore.Measurement m=store.beginMeasurement(p);store.i(p.id,41);assertTrue(store.finishMeasurement(m,900));assertEquals(900,p.ping);}
