...
public class StatsRegressionTest {
    ...
    /**
     * Mirrors the parsing loop in {@code TunnelVpnService.i.run()}: sum every outbound
     * that carries tunnelled traffic, skipping direct/block/dns-out.
     */
    private static long[] parse(String stats) {
        ...
        if (stats != null && !stats.isEmpty()) {
            for (String entry : stats.split(";")) {
                ...
                String tag = parts[0];
                if ("direct".equals(tag) || "block".equals(tag) || "dns-out".equals(tag)) {
                    continue;
                }
                long value;
                try {
    ...
    /** Chained and fragmented outbounds also carry user traffic and must be counted. */
    @Test
    public void chainAndFragmentOutboundsAreCounted() {
        long[] d = parse("proxy,uplink,100;chain,uplink,50;fragment,downlink,700;");
        assertEquals(150L, d[0]);
        assertEquals(700L, d[1]);
    }
