...
public class AppPickerActivity extends AppCompatActivity {
    ...
    public final void onPause() {
        ...
        // Per-app routing is fixed at VpnService.Builder time, so an edit made while the
        // tunnel is up does nothing until the next connect. Say so instead of letting the
        // user think the setting is broken.
        if (!hashSet.equals(this.initialSelection)) {
            ...
            if (com.parvaz.tunnel.core.TunnelVpnService.serviceRunning) {
                android.widget.Toast.makeText(
                        this, R.string.split_reconnect_hint, android.widget.Toast.LENGTH_LONG).show();
            }
        }
    }
}
