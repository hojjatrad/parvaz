...
public class TunnelVpnService extends VpnService {
    ...
    public class i implements Runnable {
        ...
        public final void run() {
            ...
            //
            // The proxy outbound is not the only one that carries user traffic: with a
            // chained profile the exit hop is "chain", and with fragmentation enabled the
            // bytes are attributed to "fragment". Sum every outbound except the ones that
            // never represent tunnelled traffic, so the counter matches reality.
            long dUp = 0;
            long dDown = 0;
            try {
                ...
                if (controller != null) {
                    ...
                    if (all != null && !all.isEmpty()) {
                        for (String entry : all.split(";")) {
                            ...
                            String tag = parts[0];
                            if ("direct".equals(tag) || "block".equals(tag)
                                    || "dns-out".equals(tag)) {
                                continue;   // not tunnelled traffic
                            }
                            long value;
                            try {
