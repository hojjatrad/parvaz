...
public final class GeoData {
 ...
 public static Set<String> tags(File file,boolean ip)throws IOException {
  ...
  Set<String> tags=new HashSet<>();try(RandomAccessFile f=new RandomAccessFile(file,"r")){
   ...
   while(f.getFilePointer()<end){long key=number(f,end);if(key!=10||++entries>100000)throw new IOException("Geo list");long stop=length(f,end);String code=null;int records=0;
    while(f.getFilePointer()<stop){key=number(f,stop);
     if(key==10){if(code!=null)throw new IOException("Duplicate code");code=text(f,stop,128);if(!code.matches("[A-Za-z0-9_@!-]{2,128}"))throw new IOException("Geo code");}
 ...
 private static void record(RandomAccessFile f,long end,boolean ip)throws IOException{
 ...
 private static String text(RandomAccessFile f,long end,int max)throws IOException{long stop=length(f,end),n=stop-f.getFilePointer();if(n<1||n>max)throw new IOException("Geo text");byte[] b=new byte[(int)n];f.readFully(b);return StandardCharsets.UTF_8.newDecoder().decode(java.nio.ByteBuffer.wrap(b)).toString();}
 private static long number(RandomAccessFile f,long end)throws IOException{long n=0;for(int shift=0;shift<63;shift+=7){if(f.getFilePointer()>=end)throw new EOFException();int b=f.readUnsignedByte();n|=(long)(b&127)<<shift;if((b&128)==0)return n;}throw new IOException("Varint limit");}
 private static long length(RandomAccessFile f,long end)throws IOException{long n=number(f,end),p=f.getFilePointer();if(n>end-p)throw new IOException("Geo length");return p+n;}
 private static void skip(RandomAccessFile f,long end,long key)throws IOException{if(key<8)throw new IOException("Geo field");switch((int)(key&7)){case 0:number(f,end);break;case 1:advance(f,end,8);break;case 2:f.seek(length(f,end));break;case 5:advance(f,end,4);break;default:throw new IOException("Geo wire type");}}
 private static void advance(RandomAccessFile f,long end,int n)throws IOException{if(n>end-f.getFilePointer())throw new EOFException();f.seek(f.getFilePointer()+n);}
}
