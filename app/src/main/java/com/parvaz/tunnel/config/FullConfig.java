...
public final class FullConfig {
 ...
 private static void validate(Object value,int depth)throws JSONException {
  ...
  for(String key:new ArrayList<String>(){{Iterator<String> it=o.keys();while(it.hasNext())add(it.next());}}){
   ...
   if(Arrays.asList("bind_interface","bind-interface","interface-name","default_interface","routing-mark","routing_mark","default_mark","network_strategy","default_network_strategy","network_type","fallback_network_type","network_is_expensive","network_is_constrained","wifi_ssid","wifi_bssid").contains(k)&&!child.toString().isEmpty()&&!child.toString().equals("false")&&!child.toString().equals("0"))throw new IllegalArgumentException("Desktop interface policy cannot be applied to Android app traffic");
