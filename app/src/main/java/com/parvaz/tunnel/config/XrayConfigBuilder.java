...
public final class XrayConfigBuilder {
    ...
    public static void a(JSONObject outbound, Profile profile, Prefs prefs) throws JSONException {
        ...
        // Nagle's algorithm holds small writes back waiting for a full segment, which
        // adds up to ~40 ms to every interactive request on top of the tunnel RTT.
        // The fragment dialer already disabled it; the main outbound never did.
    ...
    public static String b(Profile profile, Prefs prefs, Profile chain,
        ...
        // Buffer per connection, in KB (Xray multiplies by 1024 internally).
        // On ARM64 the built-in default is only 4 KB, versus 512 KB on desktop
        // platforms. That tiny buffer is a real throughput ceiling on high-latency
        // links: the core cannot keep enough bytes in flight to fill the
        // bandwidth-delay product, so downloads stall well below line rate.
        // 512 KB matches the desktop default and stays modest on a phone;
        // do NOT raise this to multi-MB, it is charged per connection.
        level8.put("bufferSize", sp.getInt("buffer_size_kb", 512));
        ...
        // Xray resolves every geosite:/geoip: reference while BUILDING the config, so a
        // rule naming a tag the installed .dat files do not contain aborts core startup
        // outright ("failed to build") rather than degrading. The bundled offline files
        // are the small -lite variants and carry only a subset of the tags, so each rule
        // below is emitted only when its data is actually present, with a literal
        // fallback where one exists.
        android.content.Context geoCtx = prefs.appContext;

        // FIRST rule, deliberately: every DNS query the core generates carries the
        // "dns_inbound" tag, but nothing used to match on it, so queries fell through
        // to the tail rule. Combined with a foreign "direct" resolver that meant
        // domestic lookups left the device in clear text while the VPN read as
        // connected. Pinning the tag to the proxy outbound closes that leak.
        JSONObject dnsRule = new JSONObject();
        dnsRule.put("type", "field");
        dnsRule.put("inboundTag", new JSONArray().put("dns_inbound"));
        dnsRule.put("outboundTag", "proxy");
        rules.put(dnsRule);

        // User domain lists come before the geo rules so an explicit choice always
        // beats a category match. Empty lists emit nothing at all.
        addDomainList(rules, sp.getString("domains_block", ""), "block");
        addDomainList(rules, sp.getString("domains_direct", ""), "direct");
        addDomainList(rules, sp.getString("domains_proxy", ""), "proxy");

        JSONObject privateIp = new JSONObject();
        privateIp.put("type", "field");
        privateIp.put("outboundTag", "direct");
        if (GeoIndex.hasGeoip(geoCtx, "private")) {
            privateIp.put("ip", new JSONArray().put("geoip:private"));
        } else {
            // RFC1918 + loopback + link-local, i.e. what geoip:private encodes.
            privateIp.put("ip", new JSONArray()
                    .put("10.0.0.0/8")
                    .put("172.16.0.0/12")
                    .put("192.168.0.0/16")
                    .put("127.0.0.0/8")
        ...
        if (GeoIndex.hasGeosite(geoCtx, "private")) {
            ...
            JSONObject localDomain = new JSONObject();
            localDomain.put("type", "field");
            localDomain.put("outboundTag", "direct");
            localDomain.put("domain", new JSONArray().put("domain:localhost").put("domain:local"));
            rules.put(localDomain);
        }

        // Ads are blocked in every mode except "global" (send-everything-through-proxy).
        if (!"global".equals(mode)) {
            JSONArray adTags = new JSONArray();
            if (GeoIndex.hasGeosite(geoCtx, "category-ads-all")) {
                adTags.put("geosite:category-ads-all");
            }
            if (GeoIndex.hasGeosite(geoCtx, "category-ads-ir")) {
                adTags.put("geosite:category-ads-ir");
            }
            // With no ad lists installed the rule is simply skipped; blocking is a
            // convenience, never a reason to refuse to connect.
            if (adTags.length() > 0) {
                JSONObject ads = new JSONObject();
                ads.put("type", "field");
                ads.put("outboundTag", "block");
                ads.put("domain", adTags);
                rules.put(ads);
            }
        }
        ...
        if ("iran_direct".equals(mode)) {
            JSONArray irDomains = new JSONArray();
            if (GeoIndex.hasGeosite(geoCtx, "category-ir")) {
                irDomains.put("geosite:category-ir");
            }
            // The .ir suffix needs no geo data and covers most Iranian destinations.
            irDomains.put("domain:ir");
            JSONObject irDomain = new JSONObject();
            irDomain.put("type", "field");
            irDomain.put("outboundTag", "direct");
            irDomain.put("domain", irDomains);
            rules.put(irDomain);

            if (GeoIndex.hasGeoip(geoCtx, "ir")) {
        ...
        try {
            ...
            for (int i = 0; i < custom.length(); i++) {
                ...
                try {
                    ...
                    android.util.Log.w("Parvaz/XrayConfigBuilde", "Exception ignored", ignored);
                }
            }
        } catch (Exception ignored) {
            android.util.Log.w("Parvaz/XrayConfigBuilde", "Exception ignored", ignored);
        ...
        // Default is now DNS-over-HTTPS. Plaintext UDP/53 was observable and
        // trivially spoofable by the network operator; the bundled core has a
        // DoH resolver, it simply was not being used.
        for (String server : sp.getString("remote_dns",
                "https://1.1.1.1/dns-query,https://dns.google/dns-query").split(",")) {
        ...
        if ("iran_direct".equals(mode)) {
            ...
            // An in-country resolver keeps Iranian names resolving to their real
            // local addresses. It used to be 8.8.8.8, which meant every domestic
            // lookup left the country in clear text.
            String directAddress = sp.getString("direct_dns", "78.157.42.100").trim();
            if (directAddress.isEmpty()) {
                directAddress = "78.157.42.100";
            }
            directDns.put("address", directAddress);
            // "port" is meaningless for DoH/DoT URLs and makes the core reject them.
            if (!directAddress.contains("://")) {
                directDns.put("port", 53);
            }
            JSONArray dnsDomains = new JSONArray();
            if (GeoIndex.hasGeosite(geoCtx, "category-ir")) {
                dnsDomains.put("geosite:category-ir");
            }
            dnsDomains.put("domain:ir");
            directDns.put("domains", dnsDomains);
            ...
            if (str8 != null && !str8.isEmpty()) {
                ...
                for (String str9 : profile.reserved.split(",")) {
                    try {
                        ...
                        android.util.Log.w("Parvaz/XrayConfigBuilde", "NumberFormatException ignored", unused);
                    }
                }
    ...
    /**
     * Turns a newline-separated domain list into one routing rule.
     *
     * <p>Bare names are matched as {@code domain:} suffixes so "example.com" also
     * covers its subdomains, which is what users expect when they type a site
     * into the list. Entries that already carry an Xray prefix
     * ({@code domain:}, {@code full:}, {@code regexp:}, {@code geosite:}) are
     * passed through untouched.
     */
    private static void addDomainList(JSONArray rules, String raw, String outboundTag)
        ...
        for (String line : raw.split("\\n")) {
            ...
            if (entry.startsWith("domain:") || entry.startsWith("full:")
                    || entry.startsWith("regexp:") || entry.startsWith("geosite:")
                    || entry.startsWith("ext:")) {
                values.put(entry);
            } else {
                values.put("domain:" + entry);
            }
        }
        if (values.length() == 0) {
            return;
