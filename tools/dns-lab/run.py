...
input_files=[LAB/'run.py',LAB/'source-pin.json',LAB/'wrapper-source-pin.json',LAB/'xray-cache-generation.patch',*sorted((LAB/'tests').glob('*.go'))]
...
result={'tested_inputs_sha256':input_hashes,'ci_commit':os.environ.get('GITHUB_SHA'),'experiment':'Xray cache generation barrier','promotion_allowed':False,'production_apk_changed':False,
 'source':xray,'wrapper_source':wrapper,'patch_sha256':digest(LAB/'xray-cache-generation.patch'),
 'go_version':subprocess.check_output(['go','version'],env=env,text=True).strip(),'stages':stages,
 'blocked_by':['UDP_TRANSACTION_ID_REUSE_REPRODUCED','NO_ENGINE_WIDE_RESOLVER_RELOAD','OTHER_ENGINE_AND_SYSTEM_CACHES_NOT_COVERED','NO_ANDROID_WRAPPER_BRIDGE_OR_NATIVE_VPN_TEST'],
 'scope':'Real pinned upstream DNS source, wrapper-root dependency resolution, synthetic/in-memory DNS fixtures; not the shipped AAR, not Android, not physical DNS/leak/battery validation.'}
(WORK/'result.json').write_text(json.dumps(result,indent=2)+'\n')
print('::notice title=DNS_CACHE_LAB_RESULT::Cache source tests/reproductions succeeded; PROMOTION_BLOCKED; baseline=1 patched=54 top-level PASS events; no APK publication')
